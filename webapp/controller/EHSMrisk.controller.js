sap.ui.define([
	...
	return Controller.extend("portalnew.controller.EHSMrisk", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf portalnew.view.EHSMrisk
		 */
		//	onInit: function() {
		//
		//	},
		...
		onAfterRendering: function() {
			...
			odata.read("/risklistSet", {
				success: function(oData, response) {
					window.console.log(oData);
					window.console.log(response);
					for (var i = 0; i <= oData.results.length; i++) {
						data.Risklist.push(oData.results[i]);
					}
		...
		onrow: function(oEvent) {
			...
			if (!this.oDialog) {
				this.oDialog = sap.ui.xmlfragment("portalnew.Frangments.risk", this);
			...
			oModel.read("/riskdetailSet('" + Recnum  + "')", {
				success: function(oData, response) {
